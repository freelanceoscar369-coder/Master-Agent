"""FileExistsAction — checks whether a file exists at a known location.
READ_ONLY. Existence (or non-existence) is the answer, not a failure —
this action only fails if the request itself is malformed. See
...
class FileExistsAction(Action):
    name = FILE_EXISTS
    description = "Check whether a file exists at a known location."
    risk_tier = RiskTier.READ_ONLY
    permission_category = PermissionCategory.READ
    expected_result = "Whether the path exists, and whether it's a file, is returned; nothing changes."
    ...
    def run(self, parameters: dict[str, Any]) -> ExecutionResult:
        ...
        return ExecutionResult(
            success=True,
            output={"path": str(target), "exists": target.exists(), "is_file": target.is_file()},
        )
