"""CopyFileAction — copies a file to a destination within the same
location, either into an existing folder (keeping its filename) or as a
new literal path. REVERSIBLE_WRITE (the source is untouched; the copy can
simply be deleted to undo). See FILESYSTEM_CAPABILITIES.md.
"""
...
class CopyFileAction(Action):
    name = COPY_FILE
    description = "Copy a file to another path or folder within a known location."
    risk_tier = RiskTier.REVERSIBLE_WRITE
    permission_category = PermissionCategory.MODIFY
    expected_result = "The source file is unchanged; a copy exists at the resolved destination."
