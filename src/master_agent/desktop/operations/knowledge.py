...
PROFILES: tuple[ApplicationOperationProfile, ...] = (
    ...
    ApplicationOperationProfile(
        key="comet",
        launch=OperationNote("Started from the per-user install path or the `comet` executable; a Chromium-family browser installed under the user's own profile rather than machine-wide."),
        focus=OperationNote("Bring the existing window to the front.", "Observed on the founder's machine: this application both loses and reclaims the foreground while other work is running, so a focus confirmed a moment ago is not evidence it is still focused."),
        close=OperationNote("Close the window the ordinary way."),
        wait_until_ready=OperationNote("The window is present and its title names the page.", "Title recognition is the mechanism proven here; a deeper readiness check is a page concern."),
        health_check=OperationNote("The window is present and its title is readable."),
        recover=OperationNote("Close and relaunch."),
        known_failure_modes=(FailureMode.NOT_RUNNING, FailureMode.WINDOW_HIDDEN, FailureMode.LOADING, FailureMode.HUNG, FailureMode.MULTIPLE_INSTANCES, FailureMode.LOGIN_REQUIRED, FailureMode.UNEXPECTED_POPUP, FailureMode.NETWORK_FAILURE),
        startup_time=StartupEstimate(StartupSpeed.FAST, (1, 3), "Chromium-family cold start"),
        preferred_launch_method=LaunchMethod.EXECUTABLE_ON_PATH,
        window_strategy=WindowStrategy.TABBED_SINGLE_WINDOW,
        automation_strategy=AutomationStrategy.BROWSER_WORKER,
        recovery_approach=RecoveryApproach.RESTART_APPLICATION,
...
_BESPOKE_RECOVERY_PLANS: tuple[ApplicationRecoveryPlan, ...] = (
    ...
    ApplicationRecoveryPlan(
        ...
        guidance=(
            ...
            (FailureMode.WINDOW_HIDDEN, RecoveryGuidance("The process is running but no window is visible.", ("bring it to the front",))),
            (FailureMode.LOADING, RecoveryGuidance("A page is still loading - a page concern, not a launch concern.", ("wait for the page, not the browser",))),
            (FailureMode.HUNG, RecoveryGuidance("A tab or the whole window is not responding.", ("close and relaunch",))),
            (FailureMode.MULTIPLE_INSTANCES, RecoveryGuidance("More than one window is open - ordinary for a tabbed browser.", ("identify the window needed by its title",))),
            (FailureMode.LOGIN_REQUIRED, RecoveryGuidance("A site being viewed requires sign-in - a page concern, not a browser fault.", ("this is expected; sign in on the page",))),
            (FailureMode.UNEXPECTED_POPUP, RecoveryGuidance("A site dialog or permission prompt is blocking interaction.", ("read it before dismissing",))),
            (FailureMode.NETWORK_FAILURE, RecoveryGuidance("A page could not load due to connectivity.", ("check connectivity", "retry the navigation"))),
        ),
    ),
    ApplicationRecoveryPlan(
